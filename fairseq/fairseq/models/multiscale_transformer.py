...
@register_model("multiscale_transformer")
class MultiscaleTransformerModel(TransformerModel):
    @staticmethod
    def add_args(parser):
        parser.add_argument(
            '--conv-kernels',
            type=str,
            default="0 0 3 3 5 5 7 7",
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return TransformerMultiscaleEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        if getattr(args, "apply_to_decoder", "False"):
            return TransformerMultiscaleDecoder(
                args,
                tgt_dict,
                embed_tokens,
...
class TransformerMultiscaleEncoder(TransformerEncoder):
    ...
    def build_encoder_layer(self, args, normal_layer=False):
        if normal_layer==True:
            ...
            layer = TransformerMultiscaleEncoderLayer(args)
...
class TransformerMultiscaleDecoder(TransformerDecoder):
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = TransformerMultiscaleDecoderLayer(args, no_encoder_attn)
        checkpoint = getattr(args, "checkpoint_activations", False)
        if checkpoint:
            offload_to_cpu = getattr(args, "offload_activations", False)
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
...
@register_model_architecture("multiscale_transformer", "multiscale_transformer")
